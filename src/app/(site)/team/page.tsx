  ...
  return (
    <>
      <PageHead
        ...
        lede="Specialists in automation, AI, e-commerce and digital marketing."
        crumbs={[{ href: '/', label: 'Home' }, { label: 'Team' }]}
      />
          ...
          <div className="index-grid">
            {items.map((item, i) => (
              <Reveal as="article" key={item.id} delay={i * 0.05}>
                <Link href={`/team/${item.slug}`} className="index-card h-full" style={{ ['--tone' as string]: item.tone }}>
